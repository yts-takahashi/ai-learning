...
- [ ] **キーボードショートカット一覧モーダル** | UX/UI | Impact:Medium | Effort:Low | `?`キーで全ショートカット（j/k/⌘K/Esc）を一覧表示するモーダルを追加
- [ ] **推奨学習時間 vs 実績比較** | 学習体験 | Impact:Medium | Effort:Low | レッスン完了時にsessionHistoryから実際の学習時間を取得して推奨時間と並べて表示
- [ ] **連続学習日数トラッカー** | 学習体験 | Impact:High | Effort:Medium | 連続学習日数を計算してダッシュボードに現在のストリークとバッジを表示
