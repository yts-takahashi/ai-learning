...
- [ ] **難易度フィルター** | UX/UI | Impact:Medium | Effort:Low | レッスン一覧に初級/中級/上級フィルターを追加
- [ ] **クイズ不正解後の関連リソース** | 学習体験 | Impact:Medium | Effort:Low | 不正解時に「詳しく学ぶ」として関連レッスンへのリンクを表示
- [ ] **学習ペース分析（LearningInsights）** | 学習体験 | Impact:High | Effort:Medium | チャプター別の学習時間・完了率・クイズ正答率からインサイトをダッシュボードに表示
