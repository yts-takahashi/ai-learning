...
- [x] **ログイン後にヘッダーのログインボタンが残る** | バグ | 認証状態の更新がヘッダーに反映されないケースがある。HeaderWrapperの状態同期を修正する
- [x] **マークダウンの表が正しく表示されない** | バグ/UI | mdxComponents の table/thead/tbody/tr/th/td コンポーネントが未定義のため装飾なしで表示されている。スタイル付きのテーブルコンポーネントを追加する
- [x] **絵文字をアイコンライブラリに置き換え** | UI改善 | ホームページ・ダッシュボード・レッスン一覧などで使われている絵文字（📖💻✅など）を Font Awesome または Heroicons の SVGアイコンに統一する
- [x] **レッスン一覧でチャプターごとに折りたたみ** | UX/UI | レッスン一覧ページのチャプターセクションをアコーディオン形式にして、クリックで開閉できるようにする（Client Component化が必要）
- [x] **ダッシュボードからチャプター別に学習を続ける** | UX/UI | ダッシュボードのチャプター進捗一覧から、各チャプターの最初の未完了レッスンへ直接遷移できるリンクを追加する
- [x] **レッスンコンテンツのローディングUX改善** | UX/パフォーマンス | MDX読み込み中にスケルトンUIを表示し、コンテンツが遅い場合でもUXが損なわれないようにする。loading.tsxの追加またはSuspenseバウンダリの活用
