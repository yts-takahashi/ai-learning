...
- [ ] **学習時間追跡** | 学習体験 | Impact:High | Effort:Medium | レッスン滞在時間をlocalStorageに記録し、ダッシュボードに総学習時間・平均時間を表示
- [ ] **クイズ履歴の詳細表示** | 学習体験 | Impact:High | Effort:Medium | ダッシュボードにレッスン別クイズ正答率・挑戦回数の一覧を表示
- [ ] **チャプター間の関連性マップ** | UX/UI | Impact:Medium | Effort:Medium | /lessonsページにMermaidで学習フロー（章間の依存関係）を可視化
